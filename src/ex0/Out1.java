 ...
 * @author sidereus
 */
public class Out1 extends OutProcessing {

    @OutNode public Map<Integer, double[]> outval1 = new HashMap<Integer, double[]>();
    @In public Map<Integer, double[]> inval;

    @Execute
    public void run() {
        for (Map.Entry<Integer, double[]> inMapEntry : inval.entrySet()) {
            Integer key = inMapEntry.getKey();
            double[] val = inMapEntry.getValue();

            addTimeStepValue(key, val[0], outval1);
        }
    }
